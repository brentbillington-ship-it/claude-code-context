...
11. **WebFetch-only competitor research (text-vibe inference)** — Symptom: an "aesthetics review" or "competitor UI comparison" written by an agent that never rendered a single page; pricing facts pulled from third-party blogs that contradict the vendor's own help-center page; deliverable confidently asserts UI details the agent could not have observed. Why: WebFetch returns text only — on JS-heavy modern marketing/pricing pages it often returns near-empty content that the agent fills from training-data guesses dressed as findings; competitor UIs cannot be reviewed without rendering them; pricing pages are often dynamically rendered behind a JS framework. Avoid: require Playwright (msedge channel — Edge already installed locally) + screenshot capture for any research task involving UI, pricing, or visual claims; pre-flight a Playwright smoke test before spawning research agents; per-agent verification gate on first vendor page; treat vendor own page > vendor help center > third-party blog and document any disagreement explicitly. Source: May 5, 2026 Gala Holdings Tracker research run (5+ compromised deliverables; invented Vyzer "Family Office tier" that doesn't exist; aesthetics review with zero actual UI observation).

12. **CCP that forbids the only methodology that works** — Symptom: research CCP includes a constraint like "no image downloads" or "no local screenshot saves" or "URLs only" for a task whose actual goal requires visual inspection; agents follow the constraint and produce text-only inferences passing as visual analysis. Why: the constraint author wrote it for privacy/clean-repo reasons without thinking through whether the work is reachable under that constraint; agents respect the constraint over the goal. Avoid: when writing a research CCP, audit each constraint against the actual deliverable — if a constraint blocks the only viable methodology, drop the constraint or reshape the deliverable. The CCP author owns this failure mode as much as the agent. Source: same Gala run — original CCP explicitly said "no image downloads" for an aesthetics agent whose entire job was visual analysis.
