...
| `AGENTS_RESEARCH_BOOTSTRAP.md` | **ACTIVE** | 2026-07-02 | 2026-07-02 | Pre-flight gate for research sessions: environment matrix, reachability probe, methodology decision tree, artifact verification, reachability log. Referenced by BB-Notes `research/07_build_ccp.md` since May; file finally exists | Sandbox network model changes enough that the probe/matrix guidance is obsolete |
| `AGENTS_PLAYWRIGHT_QA.md` | **RETIRED** | 2026-04-13 | 2026-04-13 | *(replaced by VISUAL_QA + BROWSER_BOOTSTRAP)* | — |
| `agents/senior-product-review.md` | **ACTIVE** | 2026-05-07 | 2026-07-02 | UIX + QA + research-gap reviewer for non-minor ships; cited in standing rules S1–S6 + V1-V5 | Permanent quality-gate agent — no time-boxed retirement |
...
| `agents/proposal-legal-review.md` | **ACTIVE** | 2026-05-15 | 2026-07-02 | Legal review of BW proposals vs executed NDA/MSA (IP/work-product overreach, missing carve-outs, governing-law, dispute language). Drafted for Gala Holdings v1 proposal review; reusable for any BW client | BW stops sending proposals |
| `agents/proposal-contract-manager.md` | **ACTIVE** | 2026-05-15 | 2026-07-02 | Buyer-eyes review and close-probability call; synthesizes the other three proposal lenses | BW stops sending proposals |
| `agents/proposal-pricing-qa.md` | **ACTIVE** | 2026-05-15 | 2026-07-02 | Pricing review of BW proposals (math, milestone economics, add-on ranges, competitive anchoring vs engagement research) | BW stops sending proposals |
| `agents/proposal-scope-qa.md` | **ACTIVE** | 2026-05-15 | 2026-07-02 | Scope QA of BW proposals: discovery + buyer-confirmation cross-reference; hard-boundary drift detection; ambiguity flagging | BW stops sending proposals |
...
| `playbook/agents/web-scraper.md` | **ACTIVE** | 2026-05-10 | 2026-05-10 | Multi-target scraper with robots.txt respect, retry/backoff, schema-validated output. References `playbook/templates/skills/scraping-pipeline-boilerplate/SKILL.md`. Pairs with `data-extractor` | Scraping pipelines retired |
...
| 2026-07-02 | CREATE | AGENTS_RESEARCH_BOOTSTRAP.md | Pre-flight gate for research sessions. Promotes the BB-Notes 2026-05-07 UPDATE_TO_CCC proposal, updated for the settings.local.json allowlist widening that proposal predated (R1 Playwright-first restored as primary; WebSearch demoted to cross-validation). Heals a dead inbound reference from BB-Notes research/07_build_ccp.md. |
| 2026-07-02 | UPDATE | (standing rules + RESEARCH_METHODOLOGY_V2 + scraping template) | Resolved the chromium-vs-msedge contradiction: standing rules § Browser channel now scopes rules per environment (ThinkPad scraping = installed chromium, ThinkPad visual research = msedge channel, claude.ai/code container = pre-installed Chromium, no playwright install). D8 dead pointer ("playwright-via-Edge rule above") fixed. Sandbox / Network section gained an Environment Matrix with a 2026-07-02 probe snapshot. |
| 2026-07-02 | UPDATE | agents/{senior-product-review,proposal-legal-review,proposal-contract-manager,proposal-pricing-qa,proposal-scope-qa}.md | Added official subagent YAML frontmatter (name/description/tools/model) so Claude Code can route to them by description; prose bodies unchanged. senior-product-review pinned to model: opus (judgment-heavy review gate per the Model Routing house pattern); proposal bundle runs on sonnet. V1-V5 blocks inspected for dedupe and deliberately KEPT: each is already a short agent-tuned binding to standing rules § Visual Review Discipline, not a verbatim copy. |
| 2026-07-06 | FIREWALL | (canonical-wide) | Stripped Halff-internal content that PRs #7-#10 merged into this personal/BW repo: SAML SSO runbook + Halff-Citizen-Devs/NC1016.153 from standing rules, Active Projects Halff rows, MMR/city examples in L7-L11, the scraper city taxonomy + MMR sourcing, Celina/consultant names in civil-doc-review, MMR sourcing in billing/cost, and the 07-06 MMR report. Removed `tools/build-halff-export.sh` (re-committed the dev-password literal → ccc-doctor FAIL, and embodies the superseded one-canonical-generates-Halff model; corrected model is two separate cross-pollinating repos). Generic lessons kept, genericized. web-scraper retirement trigger de-Halffed. |
